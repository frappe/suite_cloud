...
frappe.ui.form.on('Mail Account', {
	refresh(frm) {
		...
		frm.add_custom_button(__('Rotate API Key'), () => frm.trigger('rotate_api_key'), __('Credentials'))
		if (frm.doc.api_key) {
	...
	rotate_api_key(frm) {
		frappe.confirm(__('Mint a new API key for this account and revoke the current one?'), () => {
