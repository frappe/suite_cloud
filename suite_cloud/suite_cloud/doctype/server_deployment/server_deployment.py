...
class ServerDeployment(Document):
    ...
    def validate_services(self) -> None:
        ...
        services = {
            "stalwart": {
                "image": f"stalwartlabs/stalwart:{get_config('stalwart_version')}",
    ...
    def fc_filebeat_stream_setup(self) -> None:
        ...
        script_path = os.path.join(
            frappe.get_app_path("suite_cloud", "deploy", "fc"), "filebeat_stream_setup.sh"
        )
    ...
    def fc_post_deploy_ssl_setup(self, contact_email: str) -> None:
        ...
        script_path = os.path.join(
            frappe.get_app_path("suite_cloud", "deploy", "fc"), "post_deploy_ssl_setup.sh"
        )
        with open(script_path) as f:
            script_content = f.read()
