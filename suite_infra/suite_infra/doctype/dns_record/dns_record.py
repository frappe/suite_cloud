...
from suite_infra.suite_infra.doctype.dns_record.dns_provider import DNSProvider
from suite_infra.utils import enqueue_job, get_config, password_or_none, user_context
from suite_infra.utils.dns import verify_dns_record
...
class DNSRecord(Document):
    ...
    def fqdn(self) -> str:
        ...
        if not root_domain_name:
            frappe.throw(_("Please set the Root Domain Name in Suite Infra Settings."))
...
def get_dns_provider(settings: Document | None = None) -> DNSProvider | None:
    """Returns the DNS Provider configured in Suite Infra Settings, or None when there is none."""

    settings = settings or frappe.get_single("Suite Infra Settings")
