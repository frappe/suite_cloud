...
def get_config(key: str | tuple[str, ...] | None = None) -> dict[str, Any] | tuple | Any:
    """Fetches configuration values, prioritizing Suite Infra Settings over the site config.

    The site config fallback is the ``suite_infra`` dict in ``site_config.json``. Cached per
    request: the returned dict is shared, so callers must treat it as read-only.
    """

    site_conf = frappe.conf.suite_infra or {}
    settings = frappe.get_cached_doc("Suite Infra Settings")
    ...
    for k in keys:
        if k not in config:
            frappe.throw(_("Suite Infra config key '{0}' not found").format(k))
...
def log_error(title: str | None = None, message: str | None = None, **kwargs) -> None:
    """Logs an error, prefixing the title with "[Suite Infra]" so these errors can be filtered out."""
